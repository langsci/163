...
\citep{MuellerGrammix}.\todostefan{add tsdb sample sentences}
...
go and hence make language acquisition possible despite the alledged Poverty of the Stimulus.
...
The following figures show the analysis of (\mex{1}) in Minimalism and in Construction Grammar. The analysis
...
\citet{Goldberg95a,Goldberg2006a}, \citet{Tomasello2003a} and others argue for a phrasal view on argument structure
...
(intentionally, see \citew{Goldberg2006a}) are not formalized the discussion of aspects not treated
...
structure rule. \citep[\page 416]{Toivonen2013a}
\end{quote}
...
\citet*{AGT2014a} develop the respective phrasal analysis of the benefactive construction.
  ...
  The approach of \citew{AGT2014a} could be seen as a way to formalize phrasal constructional
...
\citep{SBK2012a} that are explicitly lexical. See also \citew{Croft2003a} and \citew{Goldberg2013a}
...
takes an event as sole argument. Further argument roles can be added as also predicating of the
...
\z
Agents and patients are introduced by the templates given in (\mex{1}):
...
\feat{event}) \linimp\ \upsig can be consumed by a tense predicate resulting in \upsig. That is, we
arrive at a complete semantic proof that has used all resources. The actual proof involving the templates above is given in Figure~\vref{fig:benefactive-do-proof} and will be discussed in more detail  below.
...
Lexical Mapping Theory and are given in (\mex{1}):
\eal
...
If we expand the templates for \templaten{Arg1}, \templaten{Arg2}, and \templaten{Arg3}, we get:
\eal
...
\zl
(\mex{0}a) says that either the $\sigma$ value of the \subj is \feat{arg}$_1$ or the $\sigma$ value
  of the \obltheta is \feat{arg}$_1$ or \feat{arg}$_1$ is not realized in the f-structure at all.
(\mex{0}b) says that \feat{arg}$_2$ is mapped to \subj or \obj or to nothing at all and (\mex{0}c)
...
\textbf{drew} the resource (ev \linimp d) \linimp s \linimp ev \linimp d has to be used. Since
\textbf{drew} provides ev \linimp d, the combination of the two items results in s \linimp ev
...
\linimp d and since \template{patient} + \textbf{drew} was ev \linimp d, a combination is possible
...
\begin{tabular}[t]{@{}l@{}}
     \template{Arg3}\\[.5ex]
   \pformula{\lambda x\lambda y\lambda P\lambda e.P(y)(e) ~\wedge~ \mathit{beneficiary}(e) = x}
...
generalizations.\todostefan{ncline does not work properly}
...
\vmformula[\template{Benefactive}]{\lambda x\lambda y\lambda P\lambda
  e.P(y)(e) \wedge 
     \mathit{beneficiary}(e) = x}{s \linimp g \linimp (s \linimp ev
...
\mformula{\lambda y\lambda P\lambda
  e.P(y)(e) \wedge 
     \mathit{beneficiary}(e) = sandy}{g \linimp (s \linimp ev
  \linimp d) \linimp ev \linimp d}
\]
...
\mformula{\lambda P\lambda
  e.P(godzilla)(e) \wedge 
     \mathit{beneficiary}(e) = sandy}{(s \linimp ev
  \linimp d) \linimp ev \linimp d}
\]
\[
\[
...
\formula{\lambda
  e.draw^\prime(godzilla)(kim)(e) \wedge 
     \mathit{beneficiary}(e) = sandy:ev \linimp d}
\]
\justifies
\formula{\exists e.[draw^\prime(godzilla)(kim)(e) \wedge 
     \mathit{beneficiary}(e) = sandy \wedge past(e)]:d}
\]
\justifies
...
\subsection{Inheritance-based analyses: Why does it work and where are the limits}
...
gathering of constraints (\eg inheritance).\footnote{%
  ...
  \citew{MuellerDefaults} for discussion.
}
...
\ex \textsc{sem} \ibox{1} $\wedge$ \pformula{\lambda P\lambda x\lambda e.P(e) ~\wedge~ patient(e) =
      x}{[(\upsig \feat{event}) \linimp\ 
         ...
         \upsig} $\in$ \ibox{1}
\zl
So one would say that the value of \textsc{sem} is a set \iboxb{1} and that the meaning constructor for agent
...
%% subject can be dropped in the passive, the \obj can be realized as \subj, and the \objtheta can
...
\citet{Christie2010a} assumes the following c-structure rule for transitive resultative
...
\rulenode{V\\* \up~=~\down}
\rulenode{DP\\*(\up\ \obj) = \down}
\rulenode{\{ DP|AP|PP \}\\*(\up\ \xcomp) = \down\\*(\down \subj) = (\up \obj)\\*\hspace{-3em}@RESULT-T((\up PRED FN)) }\hspace{-2ex}
}
\z
...
\oneline{%
\begin{tabular}[t]{@{}lcl@{}}
hammer & V & $\lambda e . hammer(e) : (\uparrow _{\sigma} $REL$ ) $\\
       &   & $\left( \begin{tabular}{@{~}l@{~}}
                     @TRANSITIVE(hammer) \\
                     \small $\lambda P \lambda x \lambda y \lambda e . P(e) \wedge agent(e)=x \wedge patient(e)=y $: \normalsize \\
                     $(\uparrow _{\sigma} $REL$) \linimp\ (\uparrow $SUBJ$) _{\sigma}
...
contributed by the c"=structure rule that optionally licenses a benefactive \citep[\page 81]{AGT2014a}.
...
(\mex{1}) show that the objects can be extracted with the result predicate remaining in the V$'$:
\eal
\ex[]{
...
While the extraction of the benefactive is out (\ref{ex-which-teacher-toivonen}), (\mex{1}) shows that the secondary object in
...
other sources see \citealp[\page 258]{Hudson92a-u}. Hudson reports that the extraction of the primary object of normal ditransitives is also
...
passives of normal ditransitives like (\mex{1}).
\ea[]{
...
was also made by \citew[Section~4.2]{MWArgSt} in connection with the
GPSG representation of valence.). For instance, the generalization
about productive \suffix{bar} `able' derivation in German is that it only applies to
...
Another example of derivational morphology that clearly shows that the fact that there will be an
...
\citew{Simpson83a} and \citet{Christie2015a-u} for lexical proposals in some of  these frameworks. 
...
be realized as \subj or as \obj. In the active case \argone will be the \subj and because of function argument bi-uniqueness \citep[\page 334]{BATW2015a} no other element can be the \subj and hence \argtwo has to
...
\phraserule{VP}{
\rulenode{DP\\* (\upsp \subj|\obj|\objtheta) = \down}
\rulenode{VP\\* \up~=~\down}}
\z
...
going, namely the rule in (\mex{1}), that projects the lexical verb to the VP level.
\ea
\label{LFG-v-vp-two}
\phraserule{VP}{
\rulenode{(V)\\* \up~=~\down}}
...
[CP,fit=tight
  [\cnull [weil;because]]
  [VP
    [{\begin{tabular}[t]{@{}c@{}}(\down \textsc{case} = nom) $\Rightarrow$ (\up \subj = \down)\\DP\end{tabular}} [der Vater;the father]]
    [VP
...
active sentences can be realized as \subj (\mex{0}a) or as \obj (\mex{0}b). This
...
\begin{tabular}[t]{@{}l@{~}l@{}}
{V$'$} $\to$ & \rulenode{XP*\\*\down\kern .2em $\in$ (\up\ \adj)} \rulenode{(DP)\\*(\up\ \subj) =
...
\ms{
arg-st & \sliste{ NP[\str]\ind{1}, NP[\ldat]\ind{2}, NP[\str]\ind{3} }\\[1mm]
...
g in (\mex{1}a), Sandy is the \argtwo in (\mex{1}b). Godzilla is \argthree in (\mex{1}b).
\eal
\ex 
\begin{tabular}[t]{@{}l@{}}
...
%
\ms{
arg-st & \sliste{ \ibox{1} NP[\str], NP\ind{5} } $\oplus$ \ibox{2}\\[1mm]
cont   & \ms{ ind & \ibox{3} }\\
...
\z
The highest event of the semantic representation of the output of the rule is the \type{cause} event
...
\end{tabular}}
\zl
The event variable of the \type{cause} relation is \iboxt{6}. It is also the \indv of the lexical item.
...
A more readable semantic representation that corresponds to (\mex{0}b) is given in (\mex{1}):
\ea
...
A more readable semantic representation that corresponds to (\mex{0}) is given in (\mex{1}):
\ea
...
blocking extraction and passivization. Passive is treated as the suppression of the subject. If
there is an object with structural case it is the least oblique element in the passive and therefore
it gets nominative by the Case Principle that is usually assumed. Now, if the case of the object is
